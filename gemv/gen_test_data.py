...
def write_example(size_i, size_j):
    a = (np.random.randn(size_i, size_j)).astype(np.float32)
    b = np.random.randn(size_j).astype(np.float32)
    c = a.dot(b)

    prefix = os.path.join(script_dir, f"test_{size_i}x{size_j}")
...
write_example(256, 256)
write_example(3072, 3072)
write_example(16384, 16384)
